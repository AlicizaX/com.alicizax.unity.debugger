...
{
    public class ColorParser : BasicCachedAlicizaXConsoleParser<Color>
    {
        ...
        public override Color Parse(string value)
        {
            ...
            {
                throw new ParserInputException(ZString.Concat(e.Message,
        ...
        private Color ParseRGBAColor(string value)
        {
            ...
            if (colorParts.Length < 3 || colorParts.Length > 4) { throw new FormatException(ZString.Format("Cannot parse '{0}' as a Color.", value)); }

            float ParsePart(string part)
            {
                float val = float.Parse(part);
                if (val < 0 || val > 1) { throw new FormatException(ZString.Format("{0} falls outside of the valid [0,1] range for a component of a Color.", val)); }
                return val;
            }
            ...
            {
                throw new FormatException(ZString.Format("Cannot parse '{0}' as part of a Color, it must be numerical and in the valid range [0,1].", colorParts[i]));
            }
        }
        ...
        private Color ParseHexColor(string value)
        {
            ...
            {
                throw new FormatException(ZString.Format("Cannot parse '{0}' as part of a Color as it was invalid hex.", value.Substring(2 * (1 + i), 2)));
            }
        }
    }
}
